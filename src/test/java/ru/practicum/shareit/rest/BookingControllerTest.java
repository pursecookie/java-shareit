...
public class BookingControllerTest {
    ...
    @Test
    @SneakyThrows
    void create_whenEndEqualsStart_thenStatus400() {
        bookingDtoInput.setEnd(bookingDtoInput.getStart());
    ...
    @Test
    @SneakyThrows
    void readAllBookerBookings_whenStatusUnsupported_thenStatus500() {
        mockMvc.perform(get("/bookings")
                        .header("X-Sharer-User-Id", 2L)
                        .param("state", "blabla"))
                .andExpect(status().isInternalServerError());

        Mockito.verify(bookingService, Mockito.never()).readAllBookerBookings(2L, "blabla", pageable);
    }
    ...
    @Test
    @SneakyThrows
    void readAllOwnerItemBookings_whenStatusUnsupported_thenStatus500() {
        mockMvc.perform(get("/bookings")
                        .header("X-Sharer-User-Id", 1L)
                        .param("state", "blabla"))
                .andExpect(status().isInternalServerError());

        Mockito.verify(bookingService, Mockito.never()).readAllOwnerItemBookings(1L, "blabla", pageable);
    }
